					 ...
					 if(x_data>=(chessscreenpos[i][j].x-200) && x_data<=(chessscreenpos[i][j].x+200))
						 if(y_data>=(chessscreenpos[i][j].y-100) && y_data<=(chessscreenpos[i][j].y+100))
						 {
							 ...
							 if(iChessPos[i][j]==1||iChessPos[i][j]==2)
							 {
								 ...
								 if(click_x!=-1&&click_y!=-1)//��ѡ���ӷ����仯choose another chess
								 {
...
{
	if(x_last==-1&&y_last==-1) //��x_last,y_lastΪ��ֵ����Ϊ������
	{
		if(iChessPos[x][y]==1)
			return 1;
		else
	...
	else//����Ϊ��ֵ�������x_last,y_last���ϴ��ƶ����ӵ���ɫ�жϺ����л������
	{
	...
	if(usart_1_rvfinish==1)//������źŷ�����
	{
				...
				else
				{
					...
					for(int i=0;i<5;i++)
					{
						for(int j=0;j<5;j++)
						{
							if(iChessPos[i][j]-Info[i][j]!=0) //λ�øı���
							{
